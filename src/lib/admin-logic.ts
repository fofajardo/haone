...
export async function addOfficer(
  ...
  const spreadsheetId = uiSettings.residentRecordsId;
  if (!spreadsheetId) throw new Error("Resident Records ID not configured");
...
export async function deleteOfficer(id: string) {
  ...
  if (rowIndex === -1) throw new Error("Officer not found");
...
export async function updateOfficer(id: string, data: Partial<OfficerRecord>) {
  ...
  const actualRow = rowIndex + 1; // 1-based row for A1 range
  const currentRow = rows[rowIndex];
  const newRow = [...currentRow];
  ...
  if (data.position !== undefined) newRow[OFFICER_COL.POSITION] = data.position;
  if (data.name !== undefined) newRow[OFFICER_COL.NAME] = data.name;
  if (data.nickname !== undefined) newRow[OFFICER_COL.NICKNAME] = data.nickname;
  if (data.email !== undefined) newRow[OFFICER_COL.EMAIL] = data.email;
  if (data.fbLink !== undefined) newRow[OFFICER_COL.FB_LINK] = data.fbLink;
  if (data.term !== undefined) newRow[OFFICER_COL.TERM] = data.term;
  if (data.committee !== undefined) newRow[OFFICER_COL.COMMITTEE] = data.committee;
  if (data.birthday !== undefined) newRow[OFFICER_COL.BIRTHDAY] = data.birthday;
  if (data.status !== undefined) newRow[OFFICER_COL.STATUS] = data.status;
...
export async function transitionOfficerPosition(id: string, newPosition: string) {
  ...
  // 1. Update current record status
  await updateOfficer(id, { status: OfficerStatus.CHANGED_POSITION });

  // 2. Create new entry for new position
  await addOfficer({
